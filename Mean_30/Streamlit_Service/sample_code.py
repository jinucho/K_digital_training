...
val_list = ['model', 'AGE', 'BMI', 'HEIGHT(cm)', 'WEIGHT(kg)']
list_dic = {val: pickle.loads(requests.get(url).content) for val, url in zip(val_list, pkl_urls)}
...
# 사용자 입력 관련 코드
# 예시: 사용자로부터 입력을 받는 필드
feature1 = st.text_input('이름을 입력하세요:', )
feature2 = st.text_input('나이를 입력하세요:', )
# ...
# 나머지 필드 추가
# ...
...
if st.button('당신은 당뇨일까?!'):
    if st.session_state['input_data'] is not None:
        ...
        st.write(f"당뇨병 확률: {y_prob}")
    else:
        st.warning('먼저 설문 결과를 입력해주세요!')
